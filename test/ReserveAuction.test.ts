...
describe('ReserveAuction', () => {
  ...
  async function setupAuction(tokenId, currentOwnerWallet, recipientWalletArg = null) {
    let recipientWallet = recipientWalletArg;

    if (!recipientWalletArg) {
      recipientWallet = currentOwnerWallet;
    }
    ...
    return asCurrentOwnerWalletAuction.createAuction(
      tokenId, 
      duration, 
      reservePrice, 
      currentOwnerWallet.address, 
  ...
  describe('#auction happy trail', () => {
    ...
    it('should work when a recipient is assigned', async () => {
      const market = await marketAs(creatorWallet);
      const token = await tokenAs(creatorWallet);
      const auctionAsBidder = await auctionAs(bidderWallet);
      ...
      await setupAuction(tokenId, creatorWallet, recipientWallet);
  
      let ownerOf = await token.ownerOf(tokenId);
      expect(ownerOf).eq(reserveAuctionAddress);
  
      let beforeCreatorBalance = await creatorWallet.getBalance();
      let beforeRecipientBalance = await recipientWallet.getBalance();
  
      await auctionAsBidder.createBid(tokenId, {
      ...
      const creatorAmount = await market.splitShare(
      ...
      let afterRecipientBalance = await recipientWallet.getBalance();
      expect(afterRecipientBalance.toString()).eq(
        beforeRecipientBalance.add(reservePrice).sub(creatorAmount).toString()
      );
    });

    it('should work when a recipient is passed in and seller is not creator of NFT', async () => {
      const market = await marketAs(creatorWallet);
      const token = await tokenAs(creatorWallet);
      const auctionAsBidder = await auctionAs(bidderWallet)
      ...
      await setupAuction(tokenId, otherWallet, recipientWallet);
      ...
      let afterOtherBalance = await otherWallet.getBalance()
      expect(afterOtherBalance.toString()).eq(beforeOtherBalance.toString());

      let afterRecipientBalance = await recipientWallet.getBalance()
      expect(afterRecipientBalance.toString()).eq(beforeRecipientBalance.add(reservePrice).sub(creatorAmount).toString());
