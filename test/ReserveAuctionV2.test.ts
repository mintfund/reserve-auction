  ...
  describe('#createAuction', () => {
    ...
    describe('happy path', () => {
      ...
      describe('when an auction is created', () => {
        ...
        it('should use 173184 gas', async () => {
          const receipt = await tx.wait();
          const { gasUsed } = receipt;
          expect(gasUsed.toString()).to.eq('173184');
        });
  ...
  describe('#createBid', () => {
    ...
    describe('successful cases', () => {
      describe('when there is an existing bid', () => {
        ...
        it('should use 67551 gas', async () => {
          const receipt = await tx.wait();
          const { gasUsed } = receipt;
          expect(gasUsed.toString()).to.eq('67551');
        });
      });
    });
    ...
    describe('when the transaction succeeds', () => {
      ...
      it('should cost 106350 gas', async () => {
        const receipt = await tx.wait();
        const { gasUsed } = receipt;
        expect(gasUsed.toString()).to.eq('106350');
      });
  ...
  describe('#endAuction', () => {
    ...
    describe('happy path', () => {
      describe('when there is one bidder', () => {
        ...
        it('should cost 102897 gas', () => {
          const { gasUsed } = receipt;
          expect(gasUsed.toString()).to.eq('102897');
        });
      });
    });
    ...
    describe('when the first bidder is a contract that rejects ETH and is outbid', () => {
      ...
      it('should cost 144225 gas', () => {
        const { gasUsed } = receipt;
        expect(gasUsed.toString()).to.eq('144225');
      });
    });
    ...
    describe('when the first bidder is a contract that accepts ETH but uses more gas', () => {
      ...
      it('should cost 91812 gas', () => {
        const { gasUsed } = receipt;
        expect(gasUsed.toString()).to.eq('91812');
      });
    });
    ...
    describe('when the first bidder is a contract that attempts reentrancy', () => {
      ...
      it('should cost 116661 gas', () => {
        const { gasUsed } = receipt;
        expect(gasUsed.toString()).to.eq('116661');
      });
    });
  });
});
